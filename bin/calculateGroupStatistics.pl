...
my ($inputDir, $bestRepFile, $translateFile, $groupsFile, $missingGroups, $outputFile);
...
open(my $translate, '<', $translateFile) || die "Could not open file $translateFile: $!";
my %actualToInternal;
while (my $line = <$translate>) {
    chomp $line;
    if ($line =~ /(\d+_\d+):\s(\S+).*/) {
        $actualToInternal{$2} = $1;
    }
    else {
	die "Improper file format: $line";
    }
